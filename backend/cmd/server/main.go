...
func main() {
	...
	if the_container != nil {
		routes.SetupRoutes(engine, cfg, the_container.AuthHandler, the_container.UserHandler, the_container.MentorHandler, the_container.CourseHandler, the_container.AppointmentHandler, the_container.CircleHandler, the_container.PostHandler, the_container.CommentHandler, the_container.ReviewHandler, the_container.NotificationHandler, the_container.LearningHandler)
	} else {
		// 如果数据库未连接，使用默认路由
		routes.SetupRoutes(engine, cfg, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	}
