...
func main() {
	...
	if the_container != nil {
		routes.SetupRoutes(engine, cfg, the_container.AuthHandler, the_container.UserHandler, the_container.MentorHandler)
	} else {
		// 如果数据库未连接，使用默认路由
		routes.SetupRoutes(engine, cfg, nil, nil, nil)
	}
